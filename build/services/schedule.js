        ...
        this.scheduleStacks.set(_id, node_schedule_1.default.scheduleJob(_id, schedule, async () => {
            try {
                child_process_1.exec(command, async (error, stdout, stderr) => {
                    if (error) {
                        await this.logger.info('执行任务`%s`失败，时间：%s, 错误信息：%j', name, new Date().toLocaleString(), error);
                    }
                    if (stderr) {
                        await this.logger.info('执行任务`%s`失败，时间：%s, 错误信息：%j', name, new Date().toLocaleString(), stderr);
                    }
                });
            }
            catch (error) {
